import {
  ...
  const IDL = require("./idl/solarps-old.json");
  ...
  export const getUserStateKey = (userPk: anchor.web3.PublicKey) => {
  ...
  export const isInitialized = async (wallet: anchor.Wallet) => {
    let program = getProgram(wallet);
    console.log("((((((((((((((");
    try {
      let res = await program.account.globalState.fetch(getGlobalStateKey());
      console.log("1111111111111111111");
      if ((res as any).admin.toBase58() == ADMIN_WALLET.toBase58()) {
        return true;
      }
    } catch (error) { }
    return false;
  ...
  export const initialize = async (wallet: anchor.Wallet) => {
  ...
  export const checkBalance = async () => {
    ...
    const formattedBal = vaultBal.toFixed(2);
  
    console.log("Vault balance : ", formattedBal);
    return formattedBal;
  };
  
  export const setOperator = async (wallet: anchor.Wallet) => {
    if (wallet.publicKey === null) throw new WalletNotConnectedError();
  ...
  export const setInfo = async (wallet: anchor.Wallet) => {
    if (wallet.publicKey === null) throw new WalletNotConnectedError();
  ...
  export const getGlobalData = async (wallet: anchor.Wallet) => {
    if (!wallet.publicKey) {
      return null;
    }
  ...
  export const depositReward = async (wallet: anchor.Wallet, amount: number) => {
    if (wallet.publicKey === null) throw new WalletNotConnectedError();
  ...
  export const coinFlip = async (wallet: anchor.Wallet, amount: number, type: string) => {
    if (wallet.publicKey === null) throw new WalletNotConnectedError();
    ...
    console.log("rewards : ", (userData as any).rewardAmount?.toString());
    ...
    if ((userData as any).lastSpinresult != null) {
      axios.post(`${BACKEND_URL}/set_history`, {
        walletAddress: wallet.publicKey.toString(),
        betAmount: amount,
        betType: type,
        win: (userData as any).lastSpinresult,
        tx: result,
      });
    }
  
    return (userData as any).lastSpinresult;
  };
  
  export const betSol = async (wallet: anchor.Wallet, amount: number) => {
    if (wallet.publicKey === null) throw new WalletNotConnectedError();
    let program = getProgram(wallet);
  ...
  export const getRandomParticipant = async (wallet: anchor.Wallet) => {
    let program = getProgram(wallet);
    ...
    // );
    return (allUsers[randomIndex].account as any).user.toBase58();
  };
  
  async function send(wallet: anchor.Wallet, transaction: Transaction) {
  ...
  export async function sendTransaction(wallet: anchor.Wallet, transaction: Transaction) {
